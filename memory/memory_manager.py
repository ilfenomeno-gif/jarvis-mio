...
from core.guardrails import redact_sensitive
...
def save_memory(memory: dict) -> None:
    if not isinstance(memory, dict):
        return
    memory = redact_sensitive(memory)
...
def save_session_summary(summary: str, language: str = "") -> None:
    ...
    summary = redact_sensitive((summary or "").strip())
    if not summary:
        return
    memory   = load_memory()
    ...
    if language:
        entry["language"] = redact_sensitive(language)
