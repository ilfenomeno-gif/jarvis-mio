...
def _smart_click_element(element_name: str, fallback_fn=None) -> bool:
    """Tries to click a UI element using visual grounding first, falls back to fallback_fn."""
    if _VISUAL_GROUNDING and _PYAUTOGUI:
        coords = get_element_coordinates(element_name)
        if coords:
            import pyautogui
...
def _visual_wait_for_app(app_name: str, timeout: float = 6.0) -> bool:
    """Waits for an app window to appear by scanning screen visually."""
    if _VISUAL_GROUNDING and wait_for_text:
        try:
            if wait_for_text(app_name, timeout=timeout):
                return True
        except Exception as exc:
            print(f"[SendMessage] OCR app wait failed: {exc}")
    deadline = time.time() + timeout
    while time.time() < deadline:
        if _VISUAL_GROUNDING:
            coords = get_element_coordinates(app_name)
            if coords:
                return True
        time.sleep(0.6)
    return False
...
def _desktop_send(app_name: str, receiver: str, message: str) -> str:
    ...
    # Wait for app to visually appear, then search for the receiver
    _visual_wait_for_app(app_name)
    time.sleep(0.5)

    # Try to click the search bar visually first
    search_clicked = _smart_click_element(
        "search bar",
        fallback_fn=lambda: _search_in_app(receiver)
    )
    if search_clicked and _VISUAL_GROUNDING:
        # Already clicked search, now type the receiver name
        _paste_text(receiver)
        time.sleep(1.0)
    
    # Try to click the contact result visually
    contact_found = _smart_click_element(receiver)
    if not contact_found:
        pyautogui.press("enter")
    time.sleep(0.8)

    # Try to click the message input field
    input_clicked = _smart_click_element(
        "message input field",
        fallback_fn=lambda: None
    )
    if not input_clicked:
        return f"Could not safely locate the message input in {app_name}; message not sent."
    _paste_text(message)
    time.sleep(0.2)
    pyautogui.press("enter")
    time.sleep(0.3)
