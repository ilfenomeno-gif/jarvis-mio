...
try:
    from actions.screen_processor import get_element_coordinates, find_text, wait_for_text
    _VISUAL_GROUNDING = True
except Exception:
    _VISUAL_GROUNDING = False
    get_element_coordinates = None
    find_text = None
    wait_for_text = None
    ...
    for lib in _get_steam_libraries(steam_path):
        for acf in lib.glob("appmanifest_*.acf"):
            try:
                ...
                install  = re.search(r'"installdir"\s+"([^"]+)"', content)
                if app_id and name:
                    install_dir = (lib.parent / "common" / install.group(1)
...
def _find_game_executable(game_name: str, steam_path: Path | None = None) -> Path | None:
    """Find a local Steam game executable from installed manifests."""
    steam_path = steam_path or _find_steam_path()
    if not steam_path:
        return None
    wanted = game_name.casefold().strip()
    games = _get_steam_games(steam_path)
    game = next((item for item in games if item["name"].casefold() == wanted), None)
    if game is None:
        game = next((item for item in games if wanted in item["name"].casefold()), None)
    ...
    if not root.is_dir():
        return None
    preferred = []
    if game["name"].casefold() == "victoria 3":
        preferred.append(root / "binaries" / "win64" / "v3.exe")
    preferred.extend(root.glob("*.exe"))
    preferred.extend(root.rglob("*.exe"))
    excluded = ("unins", "uninstall", "crash", "launcher", "redist", "setup")
    candidates = [p for p in dict.fromkeys(preferred)
                  if p.is_file() and not any(token in p.name.casefold() for token in excluded)]
    if not candidates:
        return None
    executable = candidates[0].resolve()
    try:
        update_memory({"notes": {
            f"steam_exe_{re.sub(r'[^a-z0-9]+', '_', game['name'].casefold()).strip('_')}": {
                "value": str(executable)
            }
        }})
    except Exception as exc:
        print(f"[GameUpdater] Could not remember executable path: {exc}")
    return executable
...
def _handle_install_dialog_pyautogui(game_name: str, best_drive: dict) -> str:
    ...
    # Try visual grounding first — click the "Install" button by detecting it on screen
    if _VISUAL_GROUNDING:
        import pyautogui
        if wait_for_text:
            try:
                wait_for_text("Install", timeout=3.0)
            except Exception as exc:
                print(f"[GameUpdater] OCR install wait failed: {exc}")
        install_btn = get_element_coordinates("Install button")
        if install_btn:
            pyautogui.click(install_btn[0], install_btn[1])
            if find_text:
                for label in ("Not enough space", "Insufficient disk space", "Yetersiz alan"):
                    try:
                        if find_text(label):
                            return f"Steam reported '{label}' while installing '{game_name}'."
                    except Exception:
                        continue
            return f"Clicked Install for '{game_name}' using visual detection."

    # Fallback: click on estimated drive dropdown and Install button positions
