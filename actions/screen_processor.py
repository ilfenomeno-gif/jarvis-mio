...
try:
    ...
    _TESSERACT_MODULE = True
except ImportError:
    _TESSERACT_MODULE = False


def _find_tesseract() -> str | None:
    if not _TESSERACT_MODULE:
        return None
    candidates = [
...
_TESSERACT = _TESSERACT_PATH is not None
if _TESSERACT:
...
class OCRText:
    """A recognized text fragment in absolute screen coordinates."""
...
def _capture_screen_image():
    if not _MSS or not _PIL:
        raise RuntimeError("mss and pillow are required for local screen OCR")
    with mss.mss() as sct:
        monitors = sct.monitors
        target = monitors[1] if len(monitors) > 1 else monitors[0]
        shot = sct.grab(target)
    return PIL.Image.frombytes("RGB", shot.size, shot.rgb), target


def read_screen_text(min_confidence: float = 45.0) -> list[OCRText]:
    """Return visible text and bounding boxes without sending the screen away."""
    if not _TESSERACT:
        return []
    image, monitor = _capture_screen_image()
    data = pytesseract.image_to_data(image, output_type=_TESSERACT_OUTPUT.DICT)
    ...
    for index, raw_text in enumerate(data.get("text", [])):
        ...
        if not text or confidence < min_confidence:
            continue
        box = (
            int(monitor["left"]) + int(data["left"][index]),
            int(monitor["top"]) + int(data["top"][index]),
            int(data["width"][index]),
            int(data["height"][index]),
        )
        results.append(OCRText(text, confidence, box))
    return results


def find_text(target: str, min_confidence: float = 45.0) -> OCRText | None:
    """Find a visible text fragment, preferring exact and then partial matches."""
    wanted = " ".join(target.casefold().split())
    if not wanted:
        return None
    matches = read_screen_text(min_confidence)
    for item in matches:
        if " ".join(item.text.casefold().split()) == wanted:
            return item
    for item in matches:
        if wanted in " ".join(item.text.casefold().split()):
            return item
    return None


def wait_for_text(target: str, timeout: float = 8.0, poll_interval: float = 0.5) -> OCRText | None:
    """Poll OCR until text appears or the timeout expires."""
    if not _TESSERACT:
        return None
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        try:
            found = find_text(target)
        except Exception as exc:
            print(f"[Vision] OCR scan failed: {exc}")
            found = None
        if found:
            return found
...
def get_element_coordinates(element_name: str) -> tuple[int, int] | None:
    """Find an element locally first, then use Gemini spatial grounding as fallback."""
    try:
        local_match = find_text(element_name)
        if local_match:
            coords = local_match.center
            print(f"[Vision] OCR found '{element_name}' at {coords}")
            return coords
    except Exception as exc:
        print(f"[Vision] Local OCR unavailable: {exc}")
    ...
    try:
        ...
        image_bytes, _ = _capture_screen()
        
        if _PIL:
            import PIL.Image
            img = PIL.Image.open(io.BytesIO(image_bytes))
        else:
            print("[Vision] PIL not installed for coordinate extraction.")
            return None
        
        prompt = f"Return the 2D bounding box coordinates [ymin, xmin, ymax, xmax] of the '{element_name}' element on this screen. Return ONLY the coordinates."
        ...
        if match:
            ...
            # Spatial coordinates are typically 0-1000
            screen_width, screen_height = pyautogui.size()
            center_x = (xmin + xmax) / 2 / 1000 * screen_width
            center_y = (ymin + ymax) / 2 / 1000 * screen_height
            print(f"[Vision] Found '{element_name}' at ({int(center_x)}, {int(center_y)})")
            return (int(center_x), int(center_y))
        else:
            print(f"[Vision] Could not parse coordinates for '{element_name}': {text}")
