...
def read_active_page(
    ...
    """Reads and summarizes the currently open browser page using visual screen analysis.
    
    If Playwright has an active session it uses DOM text. Otherwise it falls back
    to a Gemini-powered screenshot analysis of whatever is visible on screen right now.
    """
    params  = parameters or {}
    question = params.get("question", "What is on this page? Summarize it.").strip()
    browser  = params.get("browser", "").lower().strip() or None

    # ── Try Playwright DOM first (fast, accurate) ─────────────────────────────
    if _registry.has(browser):
        try:
            sess   = _registry.get(browser)
            result = sess.run(sess.get_text())
            if result and len(result) > 30:
                _log(player, f"[read_active_page] DOM text obtained ({len(result)} chars)")
                return result
        except Exception as e:
            _log(player, f"[read_active_page] DOM read failed: {e}, falling back to vision.")

    # ── Fall back to visual screenshot + Gemini ───────────────────────────────
    try:
        from actions.screen_processor import _capture_screen, _get_api_key
        ...
        result = response_obj.text.strip()
        _log(player, f"[read_active_page] Vision result: {result[:80]}")
        return result
    except Exception as e:
        msg = f"Could not read the active browser page: {e}"
        _log(player, msg)
        return msg