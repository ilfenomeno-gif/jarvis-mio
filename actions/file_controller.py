...
def _file_memory_key(name: str) -> str:
    key = re.sub(r"[^a-z0-9]+", "_", Path(name).name.casefold()).strip("_")
    return f"file_path_{key[:70] or 'unknown'}"


def _remember_file_path(path: Path) -> None:
    """Persist an exact path after a successful file discovery/read."""
    try:
        update_memory({
            "notes": {
                _file_memory_key(path.name): {"value": str(path.resolve())}
            }
        })
    except Exception as exc:
        print(f"[file] Could not remember path for {path.name}: {exc}")


def _remembered_file_path(name: str) -> Path | None:
    key = _file_memory_key(name)
    try:
        entry = load_memory().get("notes", {}).get(key, {})
...
def _resolve_file_target(path: str, name: str = "") -> Path:
    target_text = name.strip() if name.strip() else path.strip()
    explicit = ((Path(path.strip().strip('"').strip("'")).expanduser() / name)
                if name else Path(path.strip().strip('"').strip("'")).expanduser())
    if explicit.is_file():
        return explicit.resolve()
    remembered = _remembered_file_path(target_text)
    if remembered:
        return remembered
...
def read_file(path: str, name: str = "", max_chars: int = 4000) -> str:
    try:
        target = _resolve_file_target(path, name)
        if not _is_safe_path(target):
            return f"Access denied: {target}"
        if not target.exists():
