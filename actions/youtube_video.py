...
def _handle_play(parameters: dict, player) -> str:
    ...
    if video_url:
        ...
        if _open_url(video_url):
            return f"Playing first relevant result for: {query}\n{video_url}"
        fallback_url = (
            f"https://www.youtube.com/results"
            f"?search_query={quote_plus(query)}"
            f"&sp={_YT_VIDEO_FILTER}"
        ...
        return f"Could not open the first result; opened the targeted YouTube search:\n{fallback_url}"
    ...
    status = "" if opened else "; opening the browser failed"
    return f"Opened targeted YouTube search for: {query}{status} (manual selection required)\n{fallback_url}"
