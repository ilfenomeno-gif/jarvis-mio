...
def _screen_find(description: str) -> tuple[int, int] | None:
    # Prefer local OCR for visible text; Gemini remains the fallback for icons
    # and controls without a readable label.
    try:
        from actions.screen_processor import find_text
        labels = [description.strip()]
        simplified = re.sub(r"\b(button|tasto|pulsante|icon|icona)\b", "", description, flags=re.I).strip()
        if simplified and simplified.casefold() != description.strip().casefold():
            labels.append(simplified)
        for label in labels:
            match = find_text(label)
            if match:
                print(f"[ComputerControl] OCR found '{label}' at {match.center}")
                return match.center
    except Exception as exc:
        print(f"[ComputerControl] OCR screen_find unavailable: {exc}")
